...
function buildTimetable(classes, teachers, subjects, subjectAssignments, classRequirements, teacherAssignments) {
  ...
  for (const cls of classes) {
    ...
    for (const req of reqs) {
      ...
      // Determine singles and doubles count
      const totalPerWeek = req.lessons_per_week || 0
      const doublesCount = req.doubles_per_week || 0
      const singlesCount = totalPerWeek - (doublesCount * 2)
    ...
    for (const lesson of lessonQueue) {
      ...
      for (const day of DAYS) {
        for (const ts of classLessonSlots) {
          ...
          // For double lessons, ensure the NEXT period is also free
          if (lesson.isDouble) {
            const nextTs = classLessonSlots.find(s => s.period === ts.period + 1)
            if (!nextTs) continue
            const nextKey = getKey(cls.id, day, nextTs.period)
            if (classSlotMap[nextKey]) continue
          }

          // Check double-lesson rule for non-practicals (prevent accidental back-to-back same subject)
          if (!lesson.isDouble && !lesson.subject.practical) {
      ...
      // If double lesson, also place the next period
      if (lesson.isDouble) {
        const nextTs = classLessonSlots.find(s => s.period === ts.period + 1)
        if (nextTs) {
          const nextCKey = getKey(cls.id, day, nextTs.period)
          classSlotMap[nextCKey] = true

          if (assignedTeacher) {
            const nextTKey = getTKey(assignedTeacher.id, day, nextTs.period)
            teacherSlotMap[nextTKey] = true
            teacherDayCount[`${assignedTeacher.id}_${day}`] = (teacherDayCount[`${assignedTeacher.id}_${day}`] || 0) + 1
            teacherWeekCount[assignedTeacher.id] = (teacherWeekCount[assignedTeacher.id] || 0) + 1
          }
...
export default function AdminTimetablePage() {
  ...
  const addSubject = async () => {
    ...
    if (err) { setError(err.message); return }
    setSubjectForm({ name: '', code: '', category: 'core', curriculum_level: 'all', lessons_per_week: '' })
  ...
  const setClassRequirement = async (classId, subjectId, lessonsPerWeek, extra = {}) => {
  ...
  const handleSeedCBCSubjects = async () => {
    ...
    if (error) { setError('Seed failed: ' + error.message); return }
    showSuccess(`Seeded ${data} new CBC subject${data !== 1 ? 's' : ''}!`)
    fetchAll()
  }
            ...
            <div className="adm-subject-form">
              ...
              <div className="form-grid">
                ...
                <div className="form-field">
                  ...
                  <select value={subjectForm.curriculum_level}
                    onChange={e => setSubjectForm({ ...subjectForm, curriculum_level: e.target.value })}>
                    <option value="all">All Levels</option>
                    <option value="pre-primary">Pre-Primary</option>
                    <option value="lower-primary">Lower Primary (1-3)</option>
                    <option value="upper-primary">Upper Primary (4-6)</option>
                    <option value="junior">Junior School (7-9)</option>
                    <option value="senior">Senior School (10-11)</option>
                  </select>
                </div>
              </div>
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <button className="adm-tt-add-btn" onClick={handleSeedCBCSubjects} style={{ background: '#059669', color: '#fff' }}>
                  <Zap size={15} /> Seed All CBC Subjects
                </button>
              </div>
            </div>
          </div>
                  ...
                  <div key={s.id} className="adm-subject-row">
                    ...
                    <div className="adm-subject-info">
                      ...
                      <div className="adm-subject-meta">
                        ...
                        {s.curriculum_level && s.curriculum_level !== 'all' && <span className="adm-subject-cat" style={{ background: '#ede9fe', color: '#7c3aed' }}>{s.curriculum_level}</span>}
                      </div>
                    </div>
                    ...
                    <div className="adm-class-reqs">
                      ...
                      <div className="adm-req-grid">
                        {subjects.map(s => {
                          ...
                          return (
                            <div key={s.id} className="adm-req-row" style={{ flexWrap: 'wrap', gap: 4 }}>
                              ...
                              {req && req.lessons_per_week > 0 && (
                                <>
                                  <select
                                    style={{ fontSize: 10, padding: '2px 4px', borderRadius: 4, border: '1px solid #e2e8f0' }}
                                    value={req.lesson_type || 'single'}
                                    onChange={e => setClassRequirement(cls.id, s.id, req.lessons_per_week, {
                                      ...req,
                                  ...
                                  {req.lesson_type === 'double' && (
                                    <input
                                      type="number" min="0" max="5" title="Doubles per week"
                                      style={{ fontSize: 10, padding: '2px 4px', borderRadius: 4, border: '1px solid #e2e8f0', width: 36 }}
                                      value={req.doubles_per_week || 0}
                                      onChange={e => setClassRequirement(cls.id, s.id, req.lessons_per_week, {
                                        ...req,
                                        doubles_per_week: Number(e.target.value),
                                      })}
                                    />
                                  )}
                                  <label style={{ fontSize: 9, display: 'flex', alignItems: 'center', gap: 2, cursor: 'pointer' }} title="Practical lesson">
                                    <input
                                      type="checkbox"
                                      checked={req.practical || false}
                                      onChange={e => setClassRequirement(cls.id, s.id, req.lessons_per_week, {
                                        ...req,
