              ...
              {Object.entries(acByTerm).map(([term, grades]) => {
                ...
                return (
                  <div key={term} className="sp-card" style={{ padding: 16 }}>
                    ...
                    <div className="sp-table-wrap"><table className="sp-table"><thead><tr><th>Subject</th>{grp.examTypes.map(et => <th key={et}>{et}{maxMap[et] != null ? ` (${maxMap[et]})` : ''}</th>)}<th>Total /100</th><th>Level</th></tr></thead><tbody>
                      {grp.subjects.map((sub, i) => {
                        const cbe = getCBEGrade(sub.average, selectedStudent?.class || '')
                        return (
                          <tr key={sub.name}>
                            <td>{sub.name}</td>
                            ...
                            <td><span className="sp-badge blue">{cbe.band || '—'}{cbe.points != null ? ` · ${cbe.points}pts` : ''}</span></td>
                          </tr>
                        )
                      })}
                    </tbody></table></div>
