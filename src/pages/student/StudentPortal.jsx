    ...
    try {
      ...
      if (studentData) {
        const [
          ...
          studentData.class_id ? supabase.from('timetable_slots').select('*, subjects(name, code)').eq('class_id', studentData.class_id).order('day_of_week').order('start_time') : { data: [] },
          studentData.class_id ? supabase.from('assignments').select('*').eq('class_id', studentData.class_id).order('due_date', { ascending: true }) : { data: [] },
