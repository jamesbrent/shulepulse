...
-- Grades (no role column on grades - uses subquery to profiles)
SELECT _patch_rls_add_superadmin('grades', 'grades_staff_all', 'ALL',
  '(school_id = get_my_school_id() AND get_my_role() IN (''admin'', ''teacher'', ''hod'', ''deputy_admin'', ''class_teacher'')) OR (get_my_role() = ''superadmin'')');
...
-- CBC (no role column - uses subquery)
SELECT _patch_rls_add_superadmin('cbc_assessments', 'cbc_assessments_staff_all', 'ALL',
  '(school_id = get_my_school_id() AND get_my_role() IN (''admin'', ''teacher'', ''hod'', ''deputy_admin'', ''class_teacher'')) OR (get_my_role() = ''superadmin'')');

-- Notices (no role column - uses subquery)
SELECT _patch_rls_add_superadmin('notices', 'notices_staff_all', 'ALL',
  '(school_id = get_my_school_id() AND get_my_role() IN (''teacher'', ''hod'', ''class_teacher'', ''admin'', ''deputy_administrator'', ''bursar'', ''registrar'', ''reception'', ''librarian'')) OR (get_my_role() = ''superadmin'')');
...
-- Accounting (036) - these tables have no role column, use get_my_role()
SELECT _patch_rls_add_superadmin('chart_of_accounts', 'coa_finance_all', 'ALL',
  '((school_id = get_my_school_id()) AND (get_my_role() IN (''admin'', ''bursar'', ''deputy_administrator''))) OR (get_my_role() = ''superadmin'')');
...
DO $$ DECLARE t text; BEGIN
  FOREACH t IN ARRAY ARRAY['asset_categories','suppliers','fixed_assets','asset_events','asset_custody_history','asset_location_history','asset_maintenance','asset_depreciation_runs','asset_depreciation_lines','asset_documents'] LOOP
    ...
    EXECUTE format('CREATE POLICY fin_all_%s ON %I FOR ALL USING ((school_id = get_my_school_id() AND get_my_role() IN (''admin'',''bursar'',''deputy_administrator'')) OR (get_my_role() = ''superadmin''))', t, t);
