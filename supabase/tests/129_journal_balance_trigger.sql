...
--   E. balanced 3-line entry                   → accepted.
--
-- All scenarios use draft→lines→UPDATE posted so the deferred trigger has
-- lines present when SET CONSTRAINTS fires it. The whole script ROLLBACKs.
-- Synthetic year 9999.
-- ============================================================================
...
BEGIN
  ...
  VALUES (v_school, v_entry_no, CURRENT_DATE, 'AUTOTEST balanced A', 'manual', 'draft', v_profile)
  ...
  SET CONSTRAINTS trg_journal_entry_balance DEFERRED;
  ...
  BEGIN
    ...
    INSERT INTO journal_entries
      (school_id, entry_no, entry_date, description, source, status, created_by)
    VALUES (v_school, v_entry_no, CURRENT_DATE, 'AUTOTEST unbalanced B', 'manual', 'draft', v_profile)
    RETURNING id INTO v_je;
    INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit)
  ...
  INSERT INTO journal_entries
    (school_id, entry_no, entry_date, description, source, status, created_by)
  VALUES (v_school, v_entry_no, CURRENT_DATE, 'AUTOTEST balanced E', 'manual', 'draft', v_profile)
  RETURNING id INTO v_je;
  INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit)
